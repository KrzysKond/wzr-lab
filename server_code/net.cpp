...
unicast_net::unicast_net(unsigned short usPort)
{
	...
	// franc: Set non-blocking so the server can process his stuff without any clients connected
	u_long mode = 1;
...
int unicast_net::reciv(char *buffer, unsigned long *IP_Sender, unsigned short size)
{
	...
	sSize = recvfrom(sock, buffer, size, 0, (struct sockaddr*)&udpClntAddr, (socklen_t*)&ucleng);
